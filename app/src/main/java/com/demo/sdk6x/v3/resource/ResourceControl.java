...
public final class ResourceControl {
     ...
     * @param pId 父节点资源id
     */
    public void reqResList(int pType, String pId) {
        switch (pType) {
     ...
     * @param pId 父区域id
     */
    private void requestSubResFromRegion(String pId) {
        ...
        // 2.从区域获取监控点（摄像头）列表
        ret = VMSNetSDK.getInstance().getCameraListFromCtrlUnit(servAddr, loginData.getSessionID(), String.valueOf(pId), numPerPage, curPage,
                cameraList);
        ...
        PrintLog.log(null,"SubResFromRegion","failed:"+errDesc());
        PrintLog.log(null,"SubResFromRegion","servAddr"+servAddr+":getSessionID:"+loginData.getSessionID()+",pId"+pId+":");


        List<Object> allData = new ArrayList<Object>();
        allData.addAll(regionList);
        allData.addAll(cameraList);
        PrintLog.log(null,"SubResFromRegion:regionlistsize，cameralistsize:",regionList.size()+","+cameraList.size());
     ...
     * @param pId 父区域id
     */
    public void requestSubResFromRegionByPage(String pId,int page) {
        ...
        // 1.从区域获取区域列表
        boolean ret = VMSNetSDK.getInstance().getRegionListFromRegion(servAddr, loginData.getSessionID()
                , String.valueOf(pId), numPerPage, page, regionList);
        Log.i(Constants.LOG_TAG, "getRegionListFromRegion ret : " + ret);
        responseFlag = responseFlag || ret;
        if (!ret) {
            Log.e(Constants.LOG_TAG, "Invoke VMSNetSDK.getRegionListFromRegion failed:" + errDesc());
        ...
        List<CameraInfo> cameraList = new ArrayList<CameraInfo>();
        // 2.从区域获取监控点（摄像头）列表
        ret = VMSNetSDK.getInstance().getCameraListFromCtrlUnit(servAddr, loginData.getSessionID(), String.valueOf(pId), numPerPage, page,
                cameraList);
        Log.i(Constants.LOG_TAG, "getCameraListFromRegion ret : " + ret);
        responseFlag = responseFlag || ret;
        if (!ret) {
        ...
        if (callback != null) {
            callback.onMsg(responseFlag ? MsgIds.GET_SUB_F_R_SUC_MORE : MsgIds.GET_SUB_F_R_FAIL_MORE, allData);
        }
    }
     ...
     * @param pId 父控制中心id
     */
    private void requestSubResFromCtrlUnit(String pId) {
        ...
        String controlUnitID = pId;// 控制中心id
        int numPerPage = 20;// 此处取10000，表示每页获取的数量，这个数值可以根据实际情况进行修改
        ...
        PrintLog.log(null,"getCameraListFromRegion","failed:"+errDesc());
        PrintLog.log(null,"getCameraListFromRegion","servAddr"+servAddr+":getSessionID:"+loginData.getSessionID()+",pId"+pId+":");

        List<Object> allData = new ArrayList<Object>();
        allData.addAll(ctrlUnitList);
        allData.addAll(regionList);
        allData.addAll(cameraList);
        PrintLog.log(null,"SubResFromRegion:ctrlUnitList,regionlistsize，cameralistsize:",
                ctrlUnitList.size()+","+regionList.size()+","+cameraList.size());
    ...
    private void requestFirstList() {
        ...
        int numPerPage = 20;// 此处传10000，由于实际不可能有那么多，表示获取所有数据
        int curPage = 1;
        List<ControlUnitInfo> ctrlUnitList = new ArrayList<ControlUnitInfo>();
        // 获取控制中心列表
        boolean ret = VMSNetSDK.getInstance().getControlUnitList(servAddr, sessionID, String.valueOf(controlUnitID), numPerPage,
        ...
        if (ctrlUnitList != null && !ctrlUnitList.isEmpty()) {
            for (ControlUnitInfo info : ctrlUnitList) {
                Log.i(Constants.LOG_TAG, "name:" + info.getName() + ",controlUnitID:" + info.getControlUnitID() + ",parentID:"
                        + info.getParentID());
                PrintLog.log(null,"FirstList","name:" + info.getName() + ",controlUnitID:" + info.getControlUnitID() + ",parentID:"
                        + info.getParentID());
            }
        }
        ...
        PrintLog.log(null,"FirstList","failed:"+errDesc());
        PrintLog.log(null,"FirstList","servAddr"+servAddr+":getSessionID:"+loginData.getSessionID()+",controlUnitID"+controlUnitID+":");
